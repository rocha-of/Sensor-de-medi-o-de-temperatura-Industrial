...
/* Usado IA como código base com objetivo de aprimoramento foi fornecido esse código ao gemini
e solicitado que o mesmo aprimorasse, após isso foi utilizado notebooklm para corrigir eventuais erros.

// Versão Convencional (Baseada em Alocação Dinâmica Dispersa)
struct NodeDinamico {
    LeituraSensor dados;
    NodeDinamico* proximo;
    NodeDinamico(int id, double ts, double val) : dados{id, ts, val}, proximo(nullptr) {}
};

class ListaEncadeadaDinamica {
public:
    NodeDinamico* cabeca;
    ListaEncadeadaDinamica() : cabeca(nullptr) {}
    ~ListaEncadeadaDinamica() {
        while (cabeca) {
            NodeDinamico* aux = cabeca;
            cabeca = cabeca->proximo;
...
// Versão Otimizada (Memory Pool Contíguo - Alocação Manual no Array)
...
public:
    ...
    bool inserir(int id, double ts, double val) {
        ...
        int novo_no_idx = primeiro_livre;
        primeiro_livre = pool[primeiro_livre].proximo;

        pool[novo_no_idx].dados.id_sensor = id;
        pool[novo_no_idx].dados.timestamp = ts;
        pool[novo_no_idx].dados.valor = val;

        pool[novo_no_idx].proximo = cabeca;
        cabeca = novo_no_idx;
        return true;
    }

    // ITEM 1: OPERAÇÃO DE REMOÇÃO IMPLEMENTADA
    bool remover(int id) {
        int atual = cabeca;
        int anterior = -1;
        while (atual != -1) {
    ...
    void exibirLeituras(int limite = 20) {
        int atual = cabeca;
        if (atual == -1) { std::cout << "Lista vazia.\n"; return; }
        std::cout << "--- HISTÓRICO DE LEITURAS (Mostrando as " << limite << " mais recentes) ---\n";
        int contador = 0;
        while (atual != -1 && contador < limite) {
            std::cout << "Sensor ID: " << pool[atual].dados.id_sensor 
                      << " | TS: " << std::fixed << std::setprecision(0) << pool[atual].dados.timestamp 
                      << " | Valor: " << std::fixed << std::setprecision(2) << pool[atual].dados.valor << " °C\n";
            atual = pool[atual].proximo;
            contador++;
        }
    }
...
https://gemini.google.com/share/d/17Tk6pT5NmQrFxRhJr-v6Xl4pVft4lCix?usp=sharing


*/
//=============================================================================
//                              SEGUE CODIGO ATUALIZADO NOTEBOOKLM
// ============================================================================
const int MAX_NOS = 25000;
struct NodeEstatico {
    LeituraSensor dados;
...
namespace Benchmark {
    ...
    void comparacaoListas(int N = 10000) {
        ...
        long long t_trav_din = medirNS([&]() { dinamica.traversalCompleto(); });
        long long t_trav_est = medirNS([&]() { estatica.traversalCompleto(); });
 
        // — Busca —
        double v = 0;
        long long t_busca_din = medirNS([&]() { dinamica.buscar(103, v); });
        long long t_busca_est = medirNS([&]() { estatica.buscar(103, v); });
        ...
        std::cout << "\n  Justificativa teórica (para a defesa oral):\n"
                  << "  · A vantagem PRINCIPAL do pool NAO e economia de RAM, e sim LOCALIDADE.\n"
                  << "    Lista dinamica: cada 'new' devolve endereco arbitrario no heap. Ao\n"
                  << "    percorrer, a CPU salta pela RAM -> cache misses constantes.\n"
                  << "  · Pool estatico: nos num unico array contiguo. O pre-fetcher da CPU\n"
                  << "    carrega blocos sequenciais -> cache hits -> travessia mais rapida.\n"
                  << "  · Economia de RAM e bonus secundario: indice int (4B) < ponteiro (8B)\n"
                  << "    e sem overhead de allocator por no (uma so alocacao para todos).\n";
    }
